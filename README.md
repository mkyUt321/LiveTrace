...
**重要な前提——観測の全知性:** 上記の追跡成功率は、逆探知・相関エンジンが `ObservationLog` を
通じて**任意のノードで観測された任意のフローを制約なく照会できる**という前提の下での値である。
これは実質的に、ネットワークの全ノード(ひいては全リンク)に監視点を持つ集中型の防御者——ISP規模の
完全なフロー可視性——を仮定している。したがって本結果は、現実の**部分観測**下の逆探知(中継の内部
トラフィックが見えない、監視点が一部のノードにしか無い状況)に対しては**下界(楽観側)**として
読むべきであり、現実の踏み台逆探知はこれより難しい。ただし理想化しているのは観測の**カバレッジ**の
みであって識別子ではない: フローキーはローカルに見える `addr:port` の5-tuple 相当だけから構成され、
バースト・actor・連鎖の識別子は一切含まない(`contrib/livetrace/model/flow-key-util.h`)。相関エンジンは
近道を持たず、フローの結合には本物のタイミング相関を要する。この前提の設計上の含意は
「設計判断とその理由」の該当項目も参照。
...
- **観測は全ノードで完全(集中型防御者モデル)**: `ObservationLog` は全ノードの送受信イベントを
  記録し、相関エンジン・observer は任意ノードの観測フローを参照できる。これは全ノードに監視点を
  置いた集中型の防御者を模したものであり、部分的なセンサ配置(監視点が一部ノードのみ)は本バージョン
  では扱わない。この前提が主結果に与える含意(現実の部分観測下ではより難しく、本結果は下界)は
  「中心的な問い」末尾の観測の全知性に関する注記を参照。**今後の課題**: センサ配置率を規模Nと
  並ぶ第二の難易度軸として導入し、可視性を落としたときの追跡成功率の低下を測ること。
